...
uint8_t do_I2C(PCtoUTT *test) {
	...
	for (int count = 0; count < test->num_of_iterations; count++) {
		...
		HAL_I2C_Slave_Receive_IT(I2C_2, &buff_get, MAX_BUFF);
		HAL_I2C_Master_Transmit(I2C_1, I2C_SLAVE_ADRESS, &buff_send, MAX_BUFF, HAL_DELAY_Transmit_TIME);
		...
		HAL_I2C_Master_Receive_IT(I2C_1, I2C_SLAVE_ADRESS, &buff_get, MAX_BUFF);
		HAL_I2C_Slave_Transmit(I2C_2, &buff_send, MAX_BUFF, HAL_DELAY_Transmit_TIME);
		HAL_Delay(HAL_DELAY_TIME); // delay for HAL_DELAY_TIME
